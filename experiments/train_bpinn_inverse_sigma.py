...
def run_one_experiment(sigma_f, sigma_u_true, sigma_b,
    ...
    print(f"HMC dimension: {model.total_params}  "
          f"(network: {model.num_params} + k: 1 + log_sigma_u: 1)\n")
    ...
    theta_0 = model.get_initial_theta(
        k_init           = 0.5,
        log_sigma_u_init = log_sigma_u_init
    )

    print(f"Initial k        : {theta_0[model.idx_k].item():.3f}  (true: {TRUE_K})")
    print(f"Initial sigma_u  : {torch.exp(theta_0[model.idx_log_sigma_u]).item():.4f}  "
          f"(true: {sigma_u_true})\n")

\documentclass[11pt, a4paper]{article}

% Essential packages
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{geometry}
\usepackage{hyperref}
\usepackage{xcolor}
\usepackage{tcolorbox}
\usepackage{parskip} % Adds spacing between paragraphs automatically

% Layout settings
\geometry{top=2.5cm, bottom=2.5cm, left=2.5cm, right=2.5cm}

% Metadata
\title{\textbf{Reading Companion: B-PINNs (Yang et al., 2020)} \\ \large Key Insights and Personal Notes}
\author{Companion Notes}
\date{\today}

\begin{document}

\maketitle

\tableofcontents
\newpage

\section{About the Data Formulation}

\subsection*{Q: What does $\mathcal{D} = \mathcal{D}_u \cup \mathcal{D}_f \cup \mathcal{D}_b$ actually mean?}
This notation represents the total dataset available to the Bayesian framework, comprising three distinct subsets:
\begin{itemize}
    \item $\mathcal{D}_u$: Scattered noisy measurements of the state variable $u(x)$ inside the domain.
    \item $\mathcal{D}_f$: Measurements of the forcing term $f(x)$ (the source or right-hand side of the PDE).
    \item $\mathcal{D}_b$: Measurements of the boundary conditions $b(x)$.
\end{itemize}
In the Bayesian framework, every constraint is treated as data observation with associated uncertainty (noise).

\subsection*{Q: Is measuring $f$ realistic? Why not assume only measurements of $u$?}
Measuring $f$ is realistic, but it depends on the problem type:
\begin{enumerate}
    \item \textbf{Inverse Problems:} We often measure both the effect ($u$, e.g., temperature) and the cause ($f$, e.g., heat source power) to infer system parameters.
    \item \textbf{Forward Problems:} Even if $f$ is analytically known (not measured), the B-PINN framework treats it as "data" sampled at specific collocation points. These act as "virtual sensors" with a specific noise tolerance ($\sigma_f$) to allow the network flexibility.
\end{enumerate}
You \textit{can} formulate the method using only measurements of $u$, but including $\mathcal{D}_f$ is how the physics (the PDE itself) is enforced in this specific Bayesian architecture.

\section{About Where the Physics Enters}

\subsection*{Q: Where exactly does the physics appear in the network? Is it in the architecture?}
The physics is \textbf{not} in the neural network architecture (which is a standard MLP). The physics is strictly in the \textbf{Likelihood Function}.

\subsection*{Q: How does computing $\tilde{f}(x;\theta)$ make the model "physics-informed"?}
The network outputs the surrogate solution $\tilde{u}(x; \theta)$. We then use automatic differentiation to compute the residual:
\[
\tilde{f}(x; \theta) = \mathcal{N}_x(\tilde{u}(x; \theta); \lambda)
\]
The model becomes physics-informed because the likelihood $P(\mathcal{D}_f|\theta)$ penalizes the distance between this \textit{calculated} physics term $\tilde{f}$ and the \textit{observed} data $\bar{f}$:
\[
P(\mathcal{D}_f|\theta) \propto \exp\left(-\sum \frac{(\tilde{f}(x^{(i)};\theta) - \bar{f}^{(i)})^2}{2\sigma_f^2}\right)
\]
If the weights $\theta$ produce a solution $\tilde{u}$ that violates the PDE, the calculated $\tilde{f}$ will deviate from the data $\bar{f}$, resulting in a low probability.

\section{About the Squared-Error Structure}

\subsection*{Q: If the negative log-likelihood is just squared error, why is this different from the classical PINN loss?}
Mathematically, the Potential Energy function $U(\theta)$ in B-PINNs is indeed almost identical to the weighted MSE loss in standard PINNs:
\begin{align*}
    \text{PINN Loss} &\approx \text{MSE}_u + \lambda \text{MSE}_f \\
    \text{B-PINN Potential} &\approx \frac{1}{2\sigma_u^2}\text{SSE}_u + \frac{1}{2\sigma_f^2}\text{SSE}_f + \text{Prior}(\theta)
\end{align*}

\subsection*{Q: What exactly changes if the objective function looks identical?}
The difference is \textbf{Objective vs. Exploration}:
\begin{itemize}
    \item \textbf{Standard PINN:} Uses an optimizer to find the \textit{minimum} of this function (a single point $\theta^*$).
    \item \textbf{B-PINN:} Uses this function to define the \textit{geometry} of a probability distribution. We do not look for the single lowest point; we map the entire "valley floor" to understand the range of possible solutions.
\end{itemize}

\section{About Optimization vs. Sampling}

\subsection*{Q: If we do not minimize $U(\theta)$, what do we actually do computationally?}
We \textbf{integrate} (sample). We treat the parameter space $\theta$ as a physical system and simulate a particle moving through it using Hamiltonian Dynamics. This allows us to collect a set of samples $\{\theta^{(1)}, \dots, \theta^{(M)}\}$ that represent the posterior distribution.

\subsection*{Q: How do we "optimize" predictions?}
We do not optimize a single prediction. We compute the \textbf{Expected Value} via Bayesian Model Averaging:
\[
\mathbb{E}[u(x)] \approx \frac{1}{M} \sum_{i=1}^M \tilde{u}(x; \theta^{(i)})
\]
This ensemble average is often more robust to noise than any single "optimized" prediction.

\subsection*{Q: What is the role of gradients in HMC if we are not minimizing?}
In optimization, gradients tell you "which way is down." In HMC, gradients tell you "how the surface curves." This curvature information allows the sampler to maintain constant energy and traverse the high-probability region (the typical set) efficiently without getting stuck in local modes or drifting into low-probability areas.

\section{Conceptual Clarifications}

\subsection*{Q: Is this just MAP estimation with a probabilistic interpretation?}
No.
\begin{itemize}
    \item \textbf{MAP (Maximum A Posteriori):} This \textit{is} optimization. It finds the single highest peak (mode) of the posterior. Standard PINN + Regularization $\approx$ MAP.
    \item \textbf{Full Posterior (B-PINN):} This accounts for the volume of the solution space, not just the peak.
\end{itemize}

\subsection*{Q: What is the true difference between MAP and full posterior inference here?}
\begin{itemize}
    \item \textbf{MAP (Standard PINN):} Gives one answer. If data is noisy, that answer might be confidently wrong (overfitting).
    \item \textbf{Full Posterior (B-PINN):} Gives a distribution. It reveals uncertainty. If the solution is uncertain, the variance is high. Crucially, averaging over the full posterior often cancels out the noise that MAP estimation would overfit to.
\end{itemize}

\section{Why PINNs are Inherently Robust to High-Frequency Noise}

When dealing with partial differential equations (PDEs), such as the 1D Poisson equation $u_{xx} = f(x)$, a PINN network maps the coordinate $x$ to the predicted solution $u(x)$. The physics constraint enforces that the second spatial derivative of the network output must match the forcing target $f(x)$ at collocation points $x_f$.

When the forcing targets $y_f$ are heavily corrupted with noise (e.g., $y_f = -\pi^2 \sin(\pi x) + \mathcal{N}(0, 1)$), one might intuitively expect the network's prediction $u(x)$ to become noisy and jagged. However, $u(x)$ typically remains perfectly smooth. 

This occurs because solving the PDE $u_{xx} = f(x)$ mathematically implies integrating $f(x)$ twice. Integration inherently acts as a \textbf{low-pass filter}, systematically suppressing high-frequency noise. Furthermore, the spectral bias of neural networks naturally favors learning low-frequency (smooth) functions first. Consequently, the PINN effortlessly ignores high-frequency jitter in $y_f$ and fits a smooth macro-structure.

\section{Data: Boundary/Interior Sensors ($x_u, y_u$) vs. Collocation Targets ($x_f, y_f$)}

In B-PINN literature, there is a clear distinction between the actual state measurements of a system and the physical constraints acting upon it:

\begin{itemize}
    \item \textbf{Observations ($y_u$)}: These are direct sensor measurements of the true state $u(x)$. 
    \begin{itemize}
        \item If they are placed exactly on the edges of the domain, they serve strictly as \textbf{Boundary Conditions}.
        \item If they are scattered throughout the interior, the setup becomes an \textbf{Inverse Problem} or Data Assimilation scenario. Here, we are trying to deduce unknown physical parameters or forcing terms from sparse, noisy thermometer or tracking data.
    \end{itemize}
    \item \textbf{Collocation Targets ($y_f$)}: These represent the PDE constraints (e.g., the known load on a beam, or the known internal heat source of a plate). They do not measure the state $u(x)$ itself. We enforce the known PDE residual at random spatial coordinates $x_f$ to effectively bound the infinite space of possible functions to only those obeying the designated physics.
\end{itemize}

\section{The Illusion of ``Loss'' in B-PINNs: Energy Equivalence}

In standard deep learning, we optimize a neural network by minimizing a Loss Function (like Mean Squared Error). In Bayesian inference, there is no ``Loss'' function and no optimization—we merely map an energy landscape and gather samples proportionally to the \textit{Posterior Distribution}.

Therefore, the Yang et al. (2020) B-PINN paper never refers to a ``Physics Loss''. Instead, it defines the Unnormalized Posterior as a combination of independent Gaussian likelihoods:

$$ p(\theta | \mathcal{D}) \propto p(\theta) \cdot \prod \mathcal{N}\Big(y_u \mid u_\theta(x_u), \sigma_u^2\Big) \cdot \prod \mathcal{N}\Big(y_f \mid f_\theta(x_f), \sigma_f^2\Big) $$

Hamiltonian Monte Carlo (HMC) generates samples by simulating a particle sliding on an energy landscape $U(\theta)$, defined as the negative log-posterior:
$$ U(\theta) = -\log p(\theta | \mathcal{D}) $$

Because the negative natural logarithm of a Gaussian Probability Density Function mathematically expands out to the Mean Squared Error (MSE), the Bayesian ``Potential Energy'' is precisely equivalent to a scaled classical PINN Loss:
$$ -\log \mathcal{N}(y | \hat{y}, \sigma^2) = \frac{1}{2\sigma^2}(y - \hat{y})^2 + C \quad \propto \quad \text{MSE} $$

Thus, navigating the B-PINN posterior landscape is mathematically identical to traversing a surface defined by the (weighted) MSE Data Loss + MSE Physics Loss + L2 Regularization (Prior).

\section{HMC and Autograd Graph Detachment in PyTorch}

Implementing B-PINN HMC from scratch in PyTorch requires extreme care regarding computational graph tracking via \texttt{torch.autograd}.

\subsection{Why the Leapfrog Integrator requires \texttt{torch.no\_grad()}}
HMC uses a Leapfrog integrator to simulate the discrete steps of the particle across the energy landscape. 
$$ \theta_{t+\Delta t} = \theta_t + \Delta t \cdot r_t $$
If left unchecked, PyTorch's Autograd engine will silently interlink every single operation across hundreds of sampling steps, constructing a single, massive computational graph in memory. This results in memory exhaustion, and gradient explosions yielding \texttt{NaN} values. We must explicitly wrap the position ($\theta$) and momentum ($r$) updates in \texttt{with torch.no\_grad():} and \texttt{.clone().detach()} the raw tensors before the loop so PyTorch drops historical Markov chain lineage.

\subsection{Why Potential Energy $U(\theta)$ requires Autograd tracking}
While we must detach the chain history, we \textbf{cannot} use \texttt{torch.no\_grad()} to evaluate the Hamiltonian (Energy) of a given state $\theta$. 

In standard BNNs whose energy only depends on standard data outputs (e.g., classifying cats and dogs), one could evaluate energy without forming a graph. However, the B-PINN's \textit{Physics Energy} demands the calculation of spatial derivatives (e.g., $u_x, u_{xx}$). 

PyTorch computes these PDE spatial derivatives precisely by constructing the forward graph connecting spatial input $x$ to network output $u$, and then running \texttt{torch.autograd.grad(u, x, create\_graph=True)}. If \texttt{torch.no\_grad()} is active when evaluating $U(\theta)$, PyTorch leaves no graph tape to traverse, crashing the Autograd engine.

Thus, the core rhythm of PyTorch B-PINN HMC is:
\begin{enumerate}
    \item \textbf{Turn Autograd ON} to evaluate physics targets and calculate the gradient $\nabla_\theta U(\theta)$.
    \item \textbf{Turn Autograd OFF} to apply the gradient and step the physical particle forward across the landscape without building an infinitely long historical tape.
\end{enumerate}

\section{Boundary vs. Physics Competition: Trivial Solutions and Posterior Collapse}

During practical implementation of Physics-Informed Neural Networks (particularly when encountering stiff or rigid ODEs/PDEs like the unforced Damped Harmonic Oscillator, $m u_{tt} + c u_t + k u = 0$), a critical competition emerges between the Physics constraints (e.g., matching the true dynamics over time) and the Boundary/Initial data constraints ($u(0)$, $u'(0)$). 

\subsection{Standard PINNs: Optimizer Laziness and Trivial Minima}
In a standard PINN, the total loss function is typically the unweighted sum of the boundary Mean Squared Error (MSE) and the physics residual MSE. For systems where the governing equations permit a trivial solution (e.g., $u(t) = 0$ perfectly solves the unforced oscillator equation globally), the optimizer may find it overwhelmingly easier to achieve a perfect physics loss of $0.0$ by predicting a flat zero. By doing so, the network accepts a small, localized penalty for failing to intersect the exact boundary condition (e.g., $u(0)=1$) in exchange for a massive, domain-wide reduction in physics loss.

To correct this behavior, practitioners must explicitly introduce a \textbf{hyperparameter weighting factor} (e.g., $\lambda_{\text{boundary}} = 100$) scaled directly against the boundary loss. This forcefully communicates to the optimizer that minimizing the empirical data loss takes priority over physics satisfaction, demanding the network approximate the underlying PDE \textit{only on the manifold of functions} that strictly satisfy the provided boundary conditions.

\subsection{Bayesian PINNs: Energy Wells and Posterior Collapse}
Bayesian PINNs face an analogous, probabilistically equivalent challenge during Hamiltonian Monte Carlo (HMC) trajectory sampling. The HMC sampler explores the topology of the potential energy landscape defined by $U(\theta) = -\log p(\text{data} | \theta) - \log p(\text{physics} | \theta) - \log p(\theta)$. 

When the underlying physics equations are stiff or heavily constrained, randomly proposed network weights almost exclusively yield massive, non-compliant physics residuals, resulting in high potential energy and immediate sample rejection. Simultaneously, the aforementioned trivial solution $u(x)=0$ yields zero physics energy. Thus, the HMC sampler often slides inevitably into this ``zero-energy physical well'' and collapses, endlessly rejecting outgoing proposals that exhibit variance or structure. This phenomenon manifests as a flatline predictive mean coupled with a suspiciously tight, zero-bound uncertainty variance.

In a pure Bayesian formulation, one cannot arbitrarily inject a ``100x multiplier'' into the posterior density logarithm. Instead, we must control the topological structure of the energy landscape by manipulating the underlying \textbf{likelihood variances}, $\sigma_u$ (for boundary observations) and $\sigma_f$ (for physics residuals). 

By assigning a comparatively tiny variance to the boundary likelihood (e.g., $\sigma_u = 0.01$) relative to the looser physics likelihood (e.g., $\sigma_f = 0.1$), we mathematically inject a massive, incredibly steep probability density function spike perfectly centered precisely around the boundary coordinates. The HMC sampler is subsequently forced algorithmically to only explore regions of the model's parametric state-space that strictly adhere to the initial condition $u(0)=1$, successfully rescuing the Markov chain out from the trivial zero-energy collapse and accurately restoring uncertainty quantification bounding actual dynamic trajectories.







There are a few other possible extensions of Bayesian PINNs that could be interesting to pursue. 

\begin{itemize}
\item Automated Noise Inference (Learning $\sigma$)In the paper, the authors assume the fidelity of each sensor (the standard deviation of the noise $\sigma$) is known. In real-world scenarios, we rarely know exactly how noisy our sensors are.The Extension: Treat the noise scales $\sigma_u, \sigma_f,$ and $\sigma_b$ as unknown hyperparameters with their own prior distributions (e.g., an Inverse-Gamma prior).+3The Task: Use the HMC sampler to move from a parameter-only posterior $P(\theta|\mathcal{D})$ to a joint posterior $P(\theta, \sigma|\mathcal{D})$.Why it's interesting: This makes the model "Heteroscedastic," meaning the B-PINN can actually tell you which data sources it trusts more based on the physics constraint. If the data contradicts the PDE, the model will naturally infer a larger $\sigma$ for that data source.
\item 2. Physics-Informed Prior SelectionThe paper notes that the choice of priors for B-PINNs remains an "open problem" and that the current independent Gaussian priors might not be optimal.+1The Extension: Instead of a standard Gaussian prior for the weights, implement a Sparsity-Promoting Prior (like a Laplace prior or "Spike-and-Slab" prior).The Task: Apply this to an Inverse Problem where the PDE might contain many potential terms (e.g., $\mathcal{N}_x(u) = \lambda_1 u u_x + \lambda_2 u_{xx} + \dots$), and you want the Bayesian framework to perform model discovery by zeroing out the weights of irrelevant physical terms.+1Why it's interesting: This bridges the gap between B-PINNs and "Sparse Identification of Nonlinear Dynamics" (SINDy). It allows the Bayesian framework to handle model uncertainty, not just data noise.
\item The paper shows that B-PINNs handle gaps in training data much better than standard PINNs because they provide "Standard Deviation" (uncertainty) that grows where data is lacking.The Extension: Test the B-PINN on a Domain Extrapolation task.The Task: Train the model on a 1D Poisson or Allen-Cahn equation using data only from $x \in [-0.5, 0.5]$. Then, ask the model to predict the solution at $x \in [0.5, 1.0]$.+1The Comparison: Compare the B-PINN-HMC's predictive distribution against a standard PINN.+1Why it's interesting: A standard PINN will often give a high-confidence (but wrong) prediction outside the training zone. A "good" Bayesian extension would demonstrate that the B-PINN’s uncertainty bounds widen perfectly to "warn" the user that it is extrapolating beyond its knowledge.+2
\item Multifidelity scenario
    \item Active Sensor Placement: You could use the B-PINN's output variance (the blue $\pm 2\sigma$ bands in your plots) as an acquisition function. The algorithm would actively request new training data or place new collocation points only in regions where the UQ bands are the widest, massively reducing the amount of data needed to train the model.
    \item Unknown / Heteroscedastic Noise: Extend the network to jointly learn the noise scale $\sigma(x)$ alongside the solution $u(x)$. This would allow the model to automatically handle data where some regions are noisier than others without manual tuning.Robust B-PINNs (Outlier Rejection): If a sensor malfunctions and records a massive spike, a Gaussian likelihood will heavily distort the B-PINN to try and fit it. Swapping the Gaussian likelihood for a heavy-tailed distribution (like a Student-t distribution) would make the B-PINN robust to extreme outliers.
\end{itemize}

\end{document}
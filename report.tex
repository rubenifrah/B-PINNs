...
\begin{abstract}
The paper aims to extend Physics-Informed Neural Networks (PINNs) by incorporating a Bayesian framework to properly quantify uncertainty in solving froward ans inverse partial differential equation (PDE) problems, especially when data is noisy. 
While standard PINNs can learn PDE solutions using limited data and physical laws, they lack built-in uncertainty quantification. The authors propose Bayesian PINNs (B-PINNs), which treat neural network parameters probabilistically and use methods such as Hamiltonian Monte Carlo (HMC) and variational inference (VI) to estimate posterior distributions.
\end{abstract}
...
\begin{align*}
    ...
    &=  \prod_{i=1}^{N_u} \frac{\exp \left( -\frac{(\tilde{u}(\boldsymbol{x}_u^{(i)}; \boldsymbol{\theta}) - \bar{u}^{(i)})^2}{2\sigma_u^{(i)2}} \right)}{\sqrt{2\pi\sigma_u^{(i)2}}}  \prod_{i=1}^{N_f} \frac{ \exp \left( -\frac{(\tilde{f}(\boldsymbol{x}_f^{(i)}; \boldsymbol{\theta}) - \bar{f}^{(i)})^2}{2\sigma_f^{(i)2}} \right)}{\sqrt{2\pi\sigma_f^{(i)2}}} \prod_{i=1}^{N_b} \frac{\exp \left( -\frac{(\tilde{b}(\boldsymbol{x}_b^{(i)}; \boldsymbol{\theta}) - \bar{b}^{(i)})^2}{2\sigma_b^{(i)2}} \right)}{\sqrt{2\pi\sigma_b^{(i)2}}} 
\end{align*}
...
\hline
 & B-PINN & B-PINN w noise prior  & PINN & Dropout -5\% & Dropout - 1\%  \\
\hline
...
To explore the theoretical limits of Bayesian PINNs, we investigated their behavior as the neural network width scales to infinity. What exactly does a B-PINN approximate, and how does its practical implementation via HMC differ from this theoretical limit?

\subsubsection{The Infinite-Width Limit: BNNs as NNGPs}
Consider a fully-connected network with $L$ hidden layers of width $N_l$. As $N_l \to \infty$ with i.i.d. Gaussian distributions for weights and biases, the pre-activations converge to a multivariate Gaussian by the Central Limit Theorem \cite{Lee2018}. Consequently, the output surrogate function converges entirely to a Gaussian Process:
\begin{equation*}
u(\cdot;\theta) \xrightarrow[N_1, \dots, N_L \to \infty]{\text{Distribution}} \mathcal{GP}\big(0, k_{\text{NNGP}}(\cdot, \cdot)\big)
\end{equation*}
This Neural Network Gaussian Process (NNGP) proxy covariance kernel $k_{\text{NNGP}}$ is recursively determined by the network depth, variance priors ($\sigma_w^2, \sigma_b^2$), and the activation function $\phi$.

\subsubsection{Physics-Informed GPs under Linear Operators}
Assuming a linear differential operator $\mathcal{L}_x$, applying it to the Gaussian Process $u(x)$ yields another Gaussian Process. Leveraging the linearity of expectation, spatial derivatives commute with the covariance bracket, yielding exact cross-covariances linking state to physical evaluations:
\begin{equation}
...
\subsubsection{The Exact PI-GP Formulation}
By concatenating the noisy boundary and collocation observations into a block vector $Y = [\bar u, \bar f, \bar b]^T = \mathcal{A} u + \epsilon$, we construct a block covariance matrix $\mathbf{K}$ over all points. The infinite-width, physics-informed posterior field evaluated at $x^*$ is given exactly by standard GP regression:
\begin{equation}
...
\end{equation}
This mathematically guarantees a convex, resolution-free analytical posterior—bypassing the unstable non-linear landscape of HMC sampling entirely.

\subsubsection{Empirical Limitation: Expressivity vs. Adaptivity}
While mathematically rigorous, empirical testing on the highly oscillatory 1D Poisson equation ($u(x) = \sin^3(6x)$) reveals a fundamental weakness of the infinite-width NNGP setup. 
...
\paragraph{Spectral Bias and "Lazy Learning":}
Taking the width to infinity forces the network into the "lazy training" (NTK) regime. The internal feature representations become permanently frozen, resulting in a rigid, stationary kernel $k_{\text{NNGP}}$. This kernel enforces a single global smoothness length-scale everywhere, creating severe \emph{spectral bias}.

\paragraph{Noise-Induced Prior Collapse:}
Figure \ref{fig:gp_comparison} illustrates this failure under high observational noise. At $\sigma=0.1$, the noise matrix $\Sigma_{\text{noise}}$ acts as heavy Tikhonov regularization in Eq. \ref{eq:pigp_posterior}, mathematically overpowering the physics sub-covariance matrix $\mathcal{L}_x \mathcal{L}_{x'} k(X_f, X_f)$. Stripped of reliable interior data and unable to warp its frozen features to capture the sharp local sine waves, the PI-GP falls back on its zero-mean prior. This forces a simplistic linear interpolation across the domain.

\paragraph{The Power of Finite B-PINNs:}
Conversely, finite-width B-PINNs (e.g., $N=50$) easily regress this target. Finite networks perform active \emph{representation learning}. Driven by the massively penalized data likelihood ($1/\sigma^2$) during parameter updates, they actively traverse the posterior to learn completely non-stationary basis functions perfectly suited for local physical gradients. Therefore, for complex real-world phenomena, finite B-PINNs remain vastly superior.



\section{Conclusion}
This report investigated the efficacy of B-PINNs for solving forward PDE problems under noisy data conditions, benchmarking them against standard PINNs and Dropout-PINNs. Through a series of 1D PDEs, we demonstrated that B-PINNs out-perform deterministic approaches when observational noise is present. 

By employing HMC sampling to draw from the exact, unnormalized posterior distribution, B-PINNs mitigate the overfitting that plagues standard PINNs trained via gradient descent. Plus, B-PINNs provide predictive uncertainty quantification. Our reliability analysis confirmed that B-PINNs yield well-calibrated confidence intervals, accurately bounding the true underlying physics, whereas heuristic approximations like Monte Carlo Dropout exhibit over-confidence and fail to capture the true posterior variance. 

Finally, we presented a theoretical bridge between infinite-width B-PINNs and Physics-Informed Gaussian Processes (PI-GP). While the infinite-width limit offers an elegant, closed-form analytical posterior that perfectly enforces linear differential operators, our empirical evaluation uncovered its fundamental weakness: the rigid, stationary nature of the Neural Network Gaussian Process kernel prevents it from capturing highly oscillatory dynamics. We empirically showed that finite-width B-PINNs retain the capacity for feature learning. 

\subsection{Future research}
Future work could explore several promising avenues to extend the capabilities and efficiency of B-PINNs:
\begin{enumerate}
    \item \textbf{Inverse Problems and Parameter Discovery:} This study focused exclusively on forward problems, but B-PINNs are also suited for inverse problems where the physical equation parameters are unknown. Incorporating these physical constants into the HMC sampling space would allow for joint distributions over both the surrogate state function and the governing equations.
    
    \item \textbf{Advanced Variational Inference (VI):} HMC remains computationally heavy for deep architectures or dense 2D/3D spatiotemporal grids (such as the Allen-Cahn or Navier-Stokes equations). Developing robust, heavy-tailed Variational Inference schemes (such as Normalizing Flows) could offer a scalable alternative of HMC at a fraction of the computational cost.
    
    \item \textbf{Priors and Architectures:} Exploring the direct influence of alternative weight priors on the effective architecture of the network could further improve the B-PINN's ability to regress complex, multi-scale phenomena.
\end{enumerate}
...
\begin{tabular}{|c|c|c|c|c|c|}
\hline
 & B-PINN & B-PINN w noise prior  & PINN & PINN w dropout  \\
\hline

...
class BNN_Inverse(nn.Module):
    ...
    def log_prior_sigma_u(self, log_sigma_u):
        """
        Half-Normal prior on sigma_u, expressed in log-space.

        sigma_u ~ HalfNormal(scale=1): soft preference for small positive noise,
        weakly informative over a wide range.

        Change of variables sigma_u = exp(log_sigma_u) introduces Jacobian:
            log P(log_sigma_u) = log P(sigma_u) + log|d(sigma_u)/d(log_sigma_u)|
                               = log P(sigma_u) + log_sigma_u

        Dropping constants:
            log P(log_sigma_u) proportional to -0.5 * exp(2*log_sigma_u) + log_sigma_u

        The first term penalizes large sigma_u (prior pushes toward small noise).
        The second term is the Jacobian correction for the change of variables.
        Without it the prior on sigma_u would be distorted.
        """
        sigma_u = torch.exp(log_sigma_u)
        return -0.5 * sigma_u ** 2 + log_sigma_u
